...
html_data_csv_path = str(dir_path.parent.absolute()) + "/data/html_data_preprocessed.csv"
...
X_train, X_test, y_train, y_test = train_test_split(pandas_df_categorization["clean_text"],pandas_df_categorization[["Information","Procedure"]],test_size=0.2,shuffle=True)
...
path_to_information_classification_model = str(dir_path.parent.absolute()) + "/Service/Classification_models/DC_Service_Information_classification_model.joblib"

information_classification_model = load(path_to_information_classification_model)


# FITTING THE CLASSIFICATION MODEL using Logistic Regression (W2v) for information
# Predict y value for test dataset
y_predict_information = information_classification_model.predict(X_test_vectors_w2v)


path_to_procedure_classification_model = str(dir_path.parent.absolute()) + "/Service/Classification_models/DC_Service_Procedure_classification_model.joblib"

procedure_classification_model = load(path_to_procedure_classification_model)

# FITTING THE CLASSIFICATION MODEL using Logistic Regression (W2v) for procedure
# Predict y value for test dataset
y_predict_procedure = procedure_classification_model.predict(X_test_vectors_w2v)

Result_Information = np.absolute(y_test["Information"].values - y_predict_information)

Result_Procedure = np.absolute(y_test["Procedure"].values - y_predict_procedure)

print(Result_Information)
print(Result_Procedure)

Perfect_score = 0
Not_Perfect_score = 0
Not_at_all_score = 0

for i in range(len(Result_Information)):
    score = Result_Information[i] + Result_Procedure[i]
    if score == 0:
        Perfect_score+=1
    elif score == 0:
        Not_Perfect_score+=1
...
print("Perfect:")
print(Perfect_score/len(Result_Information))
print("Not perfect:")
print(Not_Perfect_score / len(Result_Information))
print("Not at all perfect:")
print(Not_at_all_score / len(Result_Information))